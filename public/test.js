...
window.onload=function(){
    ...
    for(let i=0 ;i<jsonData["countries"].length;i++){
        let county= jsonData["countries"][i]["country"];
        country.options[country.options.length] = new Option(county, county);
    }   
    country.onchange=function(){
        $('#state').find('option').remove().end().append('<option value="">Select</option>').val('whatever');
        for(let i=0 ;i<jsonData["countries"].length;i++){
            if(jsonData["countries"][i]["country"]===this.value){
                k=jsonData["countries"][i]["states"];
                for(let j=0;j<k.length;j++){
                    let s=jsonData["countries"][i]["states"][j];
                    state.options[state.options.length] = new Option(s,s);
                }
            break;
            } 
        }
